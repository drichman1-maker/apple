# Affiliate Program Applications

## Amazon Associates
- **URL:** https://affiliate-program.amazon.com/
- **Sites to register:**
  1. HealthIndex (healthindex subdomain or vercel URL)
  2. TheresMac (theresmac subdomain or vercel URL)
  3. Appliance Scout
  4. MintCondition (Pokémon)
  5. GrailWatch (Comics)
- **Requirements:** Active website, privacy policy, disclosure statement
- **Status:** Need privacy pages on all sites ✅ (in progress)

## Best Buy Affiliate
- **URL:** https://www.bestbuy.com/site/best-buy-affiliate-program/pcmcat198500050001.c?id=pcmcat198500050001
- **Sites:** TheresMac, Appliance Scout
- **Status:** Program restructuring — apply when available

## AWIN (Home Depot, Lowe's, etc.)
- **URL:** https://www.awin.com/us/
- **Sites:** Appliance Scout
- **Status:** Need established traffic first

## eBay Partner Network
- **URL:** https://partnernetwork.ebay.com/
- **Sites:** All aggregators
- **Status:** Need API exemption first ⏳ (submitted, waiting 24-48h)

## TCGplayer (Pokémon)
- **URL:** https://www.tcgplayer.com/
- **Site:** MintCondition
- **Status:** Need to apply

## Immediate Actions:
1. ✅ Add /privacy pages to all sites
2. ⏳ Deploy HealthIndex/TheresMac with privacy
3. ⏳ Apply for Amazon Associates
4. ⏳ Wait for eBay API exemption
5. ⏳ Apply for other programs

## Disclosure Statement (for all sites):
"As an Amazon Associate, we earn from qualifying purchases. [Site Name] participates in affiliate programs and may earn commissions on qualifying purchases."
